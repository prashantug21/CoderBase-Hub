...
export default function Page(){
    ...
    async function submitEvent(){
        ...
        const response = await fetch("/api/signup", {
            ...
            body: JSON.stringify({userid:user.userid,name:user.name, email: user.email, otp: otp }),
    ...
    return (
        <div className="max-w-2xl bg-white w-11/12 m-7 p-4 py-11 text-center rounded-2xl">
            ...
            <form className="flex m-8 flex-col gap-7 text-left">
                ...
                <input type="text" placeholder="Username" name="userid" onChange={(e)=>setuser({...user,userid:e.target.value})} className="text-3xl p-3 outline-none border-gray-800 border-2 rounded-lg placeholder:text-gray-600 w-full" minLength={4} maxLength={20} pattern="^[a-zA-Z0-9]+$" autoComplete="off"/>
                <div id="userid-error" className="text-red-500 hidden" ></div>
                </div>
                <div className="w-full">
                <input type="email" placeholder="Email" name="email" onChange={(e)=>setuser({...user,email:e.target.value})} className="text-3xl p-3 outline-none border-gray-800 border-2 rounded-lg placeholder:text-gray-600 w-full" autoComplete="off" />
                <div id="email-error" className="text-red-500 hidden"></div>
                </div>
                <div className="w-full">
                <input type="text" placeholder="Name" name="name" onChange={(e)=>setuser({...user,name:e.target.value})} className="text-3xl p-3 outline-none border-gray-800 border-2 rounded-lg placeholder:text-gray-600 w-full" maxLength={30} autoComplete="off" />
                ...
                {otpsent &&<div className="flex flex-col">
                    <div className="w-full">
                    <input type="text" placeholder="OTP" name="otp" onChange={(e)=>setotp(e.target.value)} className="text-3xl p-3 outline-none border-gray-800 border-2 rounded-lg placeholder:text-gray-600 w-full" minLength={6} maxLength={6} autoComplete="off"/>
                    </div>
